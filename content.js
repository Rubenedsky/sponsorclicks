...
var clubId = 3440;
var clubName = "Volleybalvereniging VC Spaarnestad";
...
function redirectShop(url, ref) {
    ...
    if (excludes.some(function(excl) {
            return ref.indexOf(excl) != -1 || url.indexOf(excl) != -1;
        }))
        return;

    // Do nothing if this is not the first page view of a website
    if (refdomain && url.indexOf(refdomain) != 0)
        return;
    ...
    // Redirect if confirmed by user
 		location.href = sponsorkliksLink.replace("{club_id}", clubId)
